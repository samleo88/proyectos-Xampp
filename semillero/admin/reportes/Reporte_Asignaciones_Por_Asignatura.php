...
class PDF extends FPDF
{
		function Header()
		{
			...
			$this->Cell(80);
			$this->Cell(60,20,'Reporte de Asignaciones por Asignatura',0,0,'C');
			$this->Ln(15);
		    ...
		    $this->Cell(20,20,'Asignaciones de la Asignatura:',0,0,'L');
		    $Asignatura = $_POST['asignatura'];
            $asignaciones = mysql_query("SELECT NombreAsignatura FROM asignaturas where idAsignatura = '$Asignatura'");
		...
		if ($Asignatura < 0) {
			 echo '<script> alert("Por Favor selecciona una asignatura para brindarte el Reporte correspondiente.");</script>';
       ...
       where asignaturas.idAsignatura  = '$Asignatura' ");
