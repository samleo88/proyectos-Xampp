...
class PDF extends FPDF
{
		function Header()
		{
			...
			$this->Cell(80);
			$this->Cell(50,20,'Reporte de Asignaciones por Docente',0,0,'C');
			$this->Ln(15);
		    ...
		    $this->Cell(20,20,'Asignaciones del Docente:',0,0,'L');
		    $docente = $_POST['docente'];
            $asignaciones = mysql_query("SELECT concat(NombresDocente, ' ' ,ApellidosDocente) as Docente FROM docentes where idDocente = '$docente'");
            while($row = mysql_fetch_row($asignaciones)){
            $NombreDocente = $row[0];

            }
		     $this->Cell(90,20, $NombreDocente, 0,0,'R');
			$this->Ln(15);
			...
			$this->Cell(40, 8, 'Contenido' ,1,0,'C');
		...
		if ($docente < 0) {
			 echo '<script> alert("Por Favor selecciona un docente para brindarte el Reporte correspondiente.");</script>';
       ...
       where docentes.idDocente = '$docente' ");
        ...
        $item = 0;
			while($asignaciones2 = mysql_fetch_array($asignaciones)){
				...
				$pdf->Cell(40, 8, utf8_decode($asignaciones2['Contenido']), 0,'C');
